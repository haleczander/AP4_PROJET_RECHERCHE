...
import { getCoefCMR, getCoefDanger, getCoefToxicite, getMasseG, getMassePureteG, getMasseRecycleeG, getMasseRecycleePureteG, getPrixEuro } from "../utils/molecules.utils";
...
export default class ReactionService {
    ...
    masseSolvants( reaction ) {
        return getSum( this.solvants( reaction ), solvant => getMasseG( solvant, true ) );
    }

    prixSolvants( reaction ) {
        return getSum( this.solvants( reaction ), solvant => getPrixEuro( solvant, true ) );
    }
    ...
    coefCMRSolvants( reaction ) {
        return getSum( this.solvants( reaction ), molecule => getCoefCMR(molecule, true) );
    }
    ...
    massePostTraitement( reaction ) {
        return getSum( this.postTraitement( reaction ), molecule => getMasseG( molecule, true ) );
    }

    prixPostTraitement( reaction ) {
        return getSum( this.postTraitement( reaction ), molecule => getPrixEuro( molecule, true ) );
    }
    ...
    coefCMRPostTraitement( reaction ) {
        return getSum( this.postTraitement( reaction ), molecule => getCoefCMR(molecule, true) );
    }
    ...
    coefCMRPurification( reaction ) {
        return getSum( this.purification( reaction ), molecule => getCoefCMR(molecule, true) );
    }

    massePurification( reaction ) {
        return getSum( this.purification( reaction ), molecule => getMasseG( molecule, true ) );
    }

    prixPurification( reaction ) {
        return getSum( this.purification( reaction ), molecule => getPrixEuro( molecule, true ) );
    }
    ...
    prixReactionPrincipale( reaction, activation = false ) {
        const prixActivation = activation ? getSum( this.activationReactionPrincipale( reaction ), getPrixEnergie ) : 0;
        return this.prixReactifs( reaction )
    ...
    prixReactionComplete( reaction, activation = false ) {
        const prixActivationPostTraitement = activation ? getSum( this.activationPostTraitement( reaction ), getPrixEnergie ) : 0;
        const prixActivationPurification = activation ? getSum( this.activationPurification( reaction ), getPrixEnergie ) : 0;
        
        return this.prixReactionPrincipale( reaction, activation ) 
