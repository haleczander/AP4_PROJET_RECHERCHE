...
describe("test CoutMassique : Reaction de Biginelli", () => {
  let reaction;
  let indicateur;
  ...
  test("Réaction Principale", () => {
    expect(indicateur.reactionPrincipale(reaction)).toBeCloseTo(0.123, 3);
  });

  test("Réaction complète", () => {
    expect(indicateur.reactionComplete(reaction)).toBeCloseTo(0.213, 3);
  });
});
