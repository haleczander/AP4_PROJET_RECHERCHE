...
describe("test PRMm : Reaction de Biginelli", () => {
  let reaction;
  let indicateur;
  ...
  test("Réaction Principale", () => {
    expect(indicateur.reactionPrincipale(reaction)).toBeCloseTo(0.0, 3);
  });

  test("Réaction complète", () => {
    expect(indicateur.reactionComplete(reaction)).toBeCloseTo(0.0, 3);
  });
});
