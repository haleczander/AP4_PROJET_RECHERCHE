...
describe("test CoefDanger : Reaction de Biginelli", () => {
  let reaction;
  let indicateur;
  ...
  test("Réaction Principale", () => {
    expect(indicateur.reactionPrincipale(reaction)).toBeCloseTo(0.9, 3);
  });

  test("Réaction complète", () => {
    expect(indicateur.reactionComplete(reaction)).toBeCloseTo(0.882, 3);
  });
});
